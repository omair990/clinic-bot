      ...
      <Box sx={{ flex: 1, overflow: "auto", px: 2, pb: 2, maxHeight: 460 }}>
        {typers.length > 0 && (
          <Typography variant="caption" color="primary"
            ...
            ✍️ assistant is replying to {typers.length} chat{typers.length > 1 ? "s" : ""}…
          </Typography>
        )}

        {activity.length === 0 ? (
          <Box sx={{ textAlign: "center", py: 7, color: "text.secondary" }}>
            <Typography variant="body2">Waiting for live messages…</Typography>
            <Typography variant="caption">Inbound and outbound WhatsApp turns appear here instantly.</Typography>
          </Box>
        ) : (
          <Stack spacing={1}>
            {activity.map((a) => {
                  ...
                  <Box sx={{ minWidth: 0, flex: 1 }}>
                    <Stack direction="row" justifyContent="space-between" spacing={1} alignItems="center">
                      ...
                      <Stack direction="row" spacing={0.5} alignItems="center" sx={{ flexShrink: 0 }}>
                        {a.needs_human && <Chip size="small" color="warning" label="needs human" sx={{ height: 18, fontSize: 10 }} />}
