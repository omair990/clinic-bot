...
`TRANSCRIBE_PROVIDERS`: Gemini → Groq Whisper → OpenRouter → OpenAI Whisper, skipping any
...
(note: OpenRouter requires a ≥$0.50 balance for any audio request).
...
| `app/providers/` | `claude` (Anthropic tool use + prompt caching), `gemini` (manual function calling), `mistral`/`groq`/`deepseek`/`openrouter`/`selfhosted` (OpenAI-compatible) |
