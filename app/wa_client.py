import logging

import httpx

from app.config import WA_ACCESS_TOKEN, WA_API_VERSION, WA_PHONE_NUMBER_ID

log = logging.getLogger(__name__)

BASE_URL = f"https://graph.facebook.com/{WA_API_VERSION}/{WA_PHONE_NUMBER_ID}"
HEADERS = {
    "Authorization": f"Bearer {WA_ACCESS_TOKEN}",
    "Content-Type": "application/json",
}


async def send_text(to: str, body: str) -> dict:
    payload = {
        "messaging_product": "whatsapp",
        "recipient_type": "individual",
        "to": to,
        "type": "text",
        "text": {"preview_url": False, "body": body[:4096]},
    }
    async with httpx.AsyncClient(timeout=20.0) as client:
        r = await client.post(f"{BASE_URL}/messages", headers=HEADERS, json=payload)
        if r.status_code >= 400:
            log.error("WA send_text failed status=%s body=%s", r.status_code, r.text)
        r.raise_for_status()
        return r.json()


async def send_buttons(to: str, body: str, buttons: list[dict]) -> dict:
    """buttons: [{"id": "...", "title": "..."}] — max 3, title ≤ 20 chars."""
    payload = {
        "messaging_product": "whatsapp",
        "to": to,
        "type": "interactive",
        "interactive": {
            "type": "button",
            "body": {"text": body[:1024]},
            "action": {
                "buttons": [
                    {"type": "reply", "reply": {"id": b["id"], "title": b["title"][:20]}}
                    for b in buttons[:3]
                ],
            },
        },
    }
    async with httpx.AsyncClient(timeout=20.0) as client:
        r = await client.post(f"{BASE_URL}/messages", headers=HEADERS, json=payload)
        if r.status_code >= 400:
            log.error("WA send_buttons failed status=%s body=%s", r.status_code, r.text)
        r.raise_for_status()
        return r.json()


async def send_list(to: str, body: str, button_label: str, rows: list[dict], header: str | None = None) -> dict:
    """rows: [{"id":..., "title":..., "description":...}] — max 10, title ≤ 24 chars."""
    section: dict = {"rows": [
        {"id": r["id"], "title": r["title"][:24], "description": (r.get("description") or "")[:72]}
        for r in rows[:10]
    ]}
    interactive: dict = {
        "type": "list",
        "body": {"text": body[:1024]},
        "action": {"button": button_label[:20], "sections": [section]},
    }
    if header:
        interactive["header"] = {"type": "text", "text": header[:60]}
    payload = {
        "messaging_product": "whatsapp",
        "to": to,
        "type": "interactive",
        "interactive": interactive,
    }
    async with httpx.AsyncClient(timeout=20.0) as client:
        r = await client.post(f"{BASE_URL}/messages", headers=HEADERS, json=payload)
        if r.status_code >= 400:
            log.error("WA send_list failed status=%s body=%s", r.status_code, r.text)
        r.raise_for_status()
        return r.json()


async def mark_read(message_id: str) -> None:
    payload = {
        "messaging_product": "whatsapp",
        "status": "read",
        "message_id": message_id,
    }
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            await client.post(f"{BASE_URL}/messages", headers=HEADERS, json=payload)
        except httpx.HTTPError as e:
            log.warning("mark_read failed: %s", e)
